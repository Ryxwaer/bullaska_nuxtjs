  ...
  <div class="container mx-auto px-10">
    ...
    <div v-else class="grid items-center lg:grid-cols-2">
      <transition name="fade">
        <div class="flex flex-col justify-between min-h-[260px] transition-opacity duration-500 ease-in-out"
             :class="{ 'opacity-0': !showDescription }">
          <div>
            <div v-html="dogs[activeSlide].title" class="text-4xl min-h-[88px]"/>
            <div v-html="dogs[activeSlide].excerpt"/>
          </div>
          <div class="text-right">
            <NuxtLink :to="`/blog/${dogs[activeSlide].slug}`"
                      class="hover:text-black hover:bg-amber-500 hover:bg-opacity-40 text-gray-800 py-2 px-4 border border-black rounded shadow">
              Čítať viac >>
            </NuxtLink>
          </div>
        </div>
